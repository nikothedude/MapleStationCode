...
/datum/disease/hypothermia/stage_act(delta_time, times_fired)
	...
	switch(stage)
		...
		if(2)
			if(DT_PROB(0.5, delta_time))
				affected_mob.emote("shiver violently")
		...
		if(3)
			...
			if(DT_PROB(1, delta_time))
				affected_mob.apply_damage(2, BURN, pick(BODY_ZONE_L_ARM))
			if(DT_PROB(1, delta_time))
				affected_mob.apply_damage(2, BURN, pick(BODY_ZONE_R_ARM))
			if(DT_PROB(1, delta_time))
				affected_mob.apply_damage(2, BURN, pick(BODY_ZONE_L_LEG))
			if(DT_PROB(1, delta_time))
				affected_mob.apply_damage(2, BURN, pick(BODY_ZONE_R_LEG))
			if(DT_PROB(3, delta_time))
				to_chat(affected_mob, span_danger("Your appendages feel numb."))
			if(DT_PROB(2, delta_time))
				to_chat(affected_mob, span_danger("You shiver more."))
