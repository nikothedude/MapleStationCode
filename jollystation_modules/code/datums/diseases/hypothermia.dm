	...
	switch(stage)
		...
		if(3)
			if(DT_PROB(1, delta_time))
				affected_mob.cause_typed_pain(pick(BODY_ZONE_L_ARM), 2, BURN)
			if(DT_PROB(1, delta_time))
				affected_mob.cause_typed_pain(BODY_ZONE_R_ARM, 2, BURN)
			if(DT_PROB(1, delta_time))
				affected_mob.cause_typed_pain(BODY_ZONE_L_LEG, 2, BURN)
			if(DT_PROB(1, delta_time))
				affected_mob.cause_typed_pain(BODY_ZONE_R_LEG, 2, BURN)
			if(DT_PROB(3, delta_time))
				to_chat(affected_mob, span_danger("Your appendages feel numb."))
			if(DT_PROB(2, delta_time))
				to_chat(affected_mob, span_danger("You shiver more."))
